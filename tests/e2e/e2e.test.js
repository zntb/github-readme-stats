...
const GIST_DATA = {
  ...
  description: "Trying to access this path on Windows 10 ver. 1803+ will breaks NTFS",
  language: "Text",
  starsCount: 1,
  forksCount: 0,
};
...
describe("Fetch Cards", () => {
  ...
  test("retrieve language card", async () => {
    ...
    await expect(
      axios.get(`${VERCEL_PREVIEW_URL}/api/top-langs/?username=${USER}&${CACHE_BURST_STRING}`),
    ).resolves.not.toThrow();
  ...
  test("retrieve gist card", async () => {
    ...
    await expect(
      axios.get(`${VERCEL_PREVIEW_URL}/api/gist?id=${GIST_ID}&${CACHE_BURST_STRING}`),
    ).resolves.not.toThrow();
